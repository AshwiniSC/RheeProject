 ...
 */
function compressImage(photoFile) {
  var compressionRate = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0.5;

  return new Promise(function (resolve) {
    var reader = new FileReader();
    reader.onload = function () {
      // image loaded from file system
      var fullImageBase64 = reader.result; // FULL RES
      var img = new Image();
      img.onload = function imgOnLoad() {
        ...
        var shrunkBase64 = canvas.toDataURL('image/jpeg', compressionRate); // Actual compression step
        var blob = dataURItoBlob(shrunkBase64); // eslint-disable-line
        var compressedFile = new File([blob], photoFile.name);
        resolve({
          shrunkBase64: shrunkBase64,
